...
#if defined(CFG_APP_IDLE_MODE_EN) && defined(CFG_IDLE_MODE_POWER_KEY)
//CountTime: 0(30us) -- 2000ms. step: 8ms
bool SystemPowerKeyIdleModeInit(uint32_t Mode, uint16_t CountTime)
{
	uint32_t PowerUpEvent = BACKUP_PowerupEventGet();
	bool     ret = FALSE;
	BAKEUP_FirstFlag = BACKUP_FristPowerOnFlagGet();
	...
	case POWERKEY_MODE_PUSH_BUTTON:
		//if(TRUE == BAKEUP_FirstFlag)
		{
			...
			BACKUP_FastPowerDownEnable();
			BACKUP_PowerKeyEnable();
#ifdef	BAKEUP_FIRST_ENTER_POWERDOWN
			if(TRUE == BAKEUP_FirstFlag)//第一次上电需要按下PowerKey
			{
				DBG("first power on\n");
			#if (POWERKEY_CNT>1000)
				int32_t Cnt = 3 * 1000 * (POWERKEY_CNT / 2);
...
bool SystemPowerKeyIdleModeWakeUpDetect(void)
{
	...
	case POWERKEY_MODE_BYPASS:
		break;
	case POWERKEY_MODE_PUSH_BUTTON:
		if(TRUE == BACKUP_PowerKeyPinStateGet()) //key is release
		{
			IsPowerKeyTrig = TRUE;
		}
		break;
