...
/**
 * @brief Initialization of PCM delay unit for 16-bit PCM data.
 ...
 *        If high_quality is set 1, the buffer capacity = "max_delay_samples*2*num_channels" in bytes for 16 bit PCM
 *           For example when PCM is 16 bit, max_delay_samples = 22050, then the buffer capacity should be 88200 bytes (22050*2*2) for stereo input or 44100 bytes (22050*2*1) for mono input.
 ...
 */
int32_t pcm_delay_init16(PCMDelay* ct, int32_t num_channels, int32_t max_delay_samples, int32_t high_quality, uint8_t *s);


/**
 * @brief Initialization of PCM delay unit for 24-bit PCM data.
 * @param ct Pointer to the PCM delay context structure. The structure's memory should be allocated by the calling process.
 * @param num_channels Number of channels. Both 1 channel and 2 channels are supported.
 * @param max_delay_samples Maximum delay in samples. For example if you want to have maximum 500ms delay at 44.1kHz sample rate, the max_delay_samples = delay time*sample rate = 500*44.1 = 22050.
 ...
 * @param s Delay buffer pointer. This buffer should be allocated by the caller and its capacity depends on "num_channels", "high_quality" and "max_delay_samples".
 *        If high_quality is set 1, the buffer capacity = "max_delay_samples*4*num_channels" in bytes for 24 bit PCM.
 *           For example when PCM is 24 bit, max_delay_samples = 22050, then the buffer capacity should be 176400 bytes (22050*4*2) for stereo input or 88200 bytes (22050*4*1) for mono input.
 *        If high_quality is set 0, the buffer capacity = "ceil(max_delay_samples/32)*19*num_channels" in bytes. For example if max_delay_samples = 22050, then the buffer capacity should be 26220 bytes (ceil(22050/32)*19*2) for stereo input or 13110 (ceil(22050/32)*19*1) for mono input.
 * @return error code. PCMDELAY_ERROR_OK means successful, other codes indicate error.
 */
int32_t pcm_delay_init24(PCMDelay* ct, int32_t num_channels, int32_t max_delay_samples, int32_t high_quality, uint8_t *s);
 ...
 * @return error code. PCMDELAY_ERROR_OK means successful, other codes indicate error.
 */
int32_t pcm_delay_apply16(PCMDelay* ct, int16_t *pcm_in, int16_t *pcm_out, int32_t n, int32_t delay_samples);
